...
public class MyAdaptiveBiasedMutationOperator implements GeneticOperator {
    ...
    public MyAdaptiveBiasedMutationOperator(Configuration configuration, double mutationRate) {
    ...
    private void mutateChromosome(IChromosome chromosome) {
    	double coefficient = 1 / -Math.log(1 - chromosome.getFitnessValue());
    	for (Gene gene : chromosome.getGenes()) {
    		if (random.nextDouble() < mutationRate) {
    			Double newAllele = (Double)gene.getAllele() + coefficient * random.nextGaussian();
